...
static int num_change_steps = 0;
static float delta_left, delta_right;
static float current_left, current_right;
static int change_speed_step = 1;
static unsigned long change_speed_next;
static int target_new_left_speed, target_new_right_speed;
...
void change_speed(int new_left_speed, int new_right_speed, int max_duration_in_ms)
{
  ...
  target_new_left_speed = new_left_speed;
  target_new_right_speed = new_right_speed;
  ...
  delta_left = (new_left_speed - start_left_speed) / (float)num_change_steps;
  delta_right = (new_right_speed - start_right_speed) / (float)num_change_steps;

  current_left = start_left_speed;
  current_right = start_right_speed;

  change_speed_step = 0;
  change_speed_next = millis();
}

void tick_change_speed()
{
  if ((change_speed_step <= num_change_steps) &&
      (millis() >= change_speed_next))
  {
    if (change_speed_step == num_change_steps) 
    {
      stepper_speed(MOTOR_LEFT, target_new_left_speed);
      stepper_speed(MOTOR_RIGHT, target_new_right_speed);
      left_speed = target_new_left_speed;
      right_speed = target_new_right_speed;
    }
    else
    {
      left_speed = (int)(current_left + 0.5);
      right_speed = (int)(current_right + 0.5);
      stepper_speed(MOTOR_LEFT, left_speed);
      stepper_speed(MOTOR_RIGHT, right_speed);
...
{
  enable_steppers();
  change_speed(left_speed + SPEED_MULTIPLIER, right_speed + SPEED_MULTIPLIER, INT_MAX);
}
...
void print_report()
{
  ...
  Serial.print(" ");
  Serial.print(motors_disabled);   
  Serial.print(" ");
  Serial.println(obstacle_blocking);   
}
...
void handle_red_switch()
{
  ...
  {
    if (red_switch_counter < 3) red_switch_counter++;
...
int read_signed_two_digits()
{
  ...
  char c2 = Serial.read();
  int val = sign * ((c1 - '0') * 10 + c2 - '0');
  return val;
}

void mato_set_speed()
{
  int left = read_signed_two_digits();
  int right = read_signed_two_digits();
  change_speed(SPEED_MULTIPLIER * left, SPEED_MULTIPLIER * right, INT_MAX); 
}
